...
print("image size >> ", image.shape) # 사이즈 찍기

mask = np.zeros(image.shape[:2], np.uint8)
mask[60:160, 20:120] = 255          # 관심 영역에 값(255) 할당

sum_value = cv2.sumElems(image)     # 채널 합 구하기 - 튜플로 변환
mean_value1 = cv2.mean(image)       # 채널별 평균 - 튜플로 변환
mean_value2 = cv2.mean(image, mask)


print("sum_value 자료형 : ", type(sum_value), type(sum_value[0]))  # 열과 행렬의 자료형
